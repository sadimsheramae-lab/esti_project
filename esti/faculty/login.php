...
if (isset($_SESSION['faculty_id'])) { header('Location: '.FACULTY_URL.'dashboard.php'); exit(); }
$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($faculty_id && $password) {
        ...
        $stmt->bind_param('s', $faculty_id); $stmt->execute();
        $faculty = $stmt->get_result()->fetch_assoc();
        if ($faculty && $faculty['password'] && password_verify($password, $faculty['password'])) {
            $_SESSION['faculty_id']   = $faculty['faculty_id'];
            $_SESSION['faculty_name'] = $faculty['first_name'].' '.$faculty['last_name'];
            $_SESSION['faculty_dept'] = $faculty['dept_code'];
            header('Location: '.FACULTY_URL.'dashboard.php'); exit();
        } else { $error = 'Invalid Faculty ID or Password.'; }
    } else { $error = 'Please enter your Faculty ID and Password.'; }
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Faculty Login | ESTI CGMS</title>
...
.brand-mark .mark-icon{
  ...
  font-size:19px;color:var(--green-accent);
}
...
<div class="brand-panel">
    <div class="brand-top">
        <div class="brand-mark">
            <div class="mark-icon"><i class="fas fa-graduation-cap"></i></div>
            <div class="mark-text"><strong>ESTI</strong>College Grading<br>Management System</div>
        </div>
        <h1 class="brand-heading">Where teaching<br><em>meets record.</em></h1>
        <p class="brand-sub">Sign in to manage class lists, submit grades, and update student records.</p>
    </div>
...
<div class="form-panel">
    <div class="form-wrap">
        <div class="role-switch">
            <a href="<?= BASE_URL ?>index.php">Admin</a>
            <a href="<?= BASE_URL ?>student/login.php">Student</a>
            <span class="active">Faculty</span>
        </div>

        <div class="form-eyebrow">Faculty access</div>
        <h2 class="form-title">Welcome back</h2>
        <p class="form-desc">Enter your credentials to manage your classes and grades.</p>
        ...
        <form method="POST">
            <div class="field">
                ...
                <div class="field-input">
                    ...
                    <input type="text" name="faculty_id" id="faculty_id" placeholder="Enter your Faculty ID (e.g. F0001)" value="<?= htmlspecialchars($_POST['faculty_id'] ?? '') ?>" required autofocus>
                </div>
            </div>
            <div class="field">
        ...
        <div class="form-foot-note">Default password: <strong>password</strong></div>
    </div>
</div>

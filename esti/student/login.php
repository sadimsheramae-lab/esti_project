...
if (isset($_SESSION['student_id'])) { header('Location: '.STUDENT_URL.'dashboard.php'); exit(); }
$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id_number = trim($_POST['id_number'] ?? '');
    $password  = $_POST['password'] ?? '';
    if ($id_number && $password) {
        $stmt = $conn->prepare("SELECT * FROM students WHERE id_number = ? AND status = 'Active'");
        $stmt->bind_param('s', $id_number); $stmt->execute();
        $student = $stmt->get_result()->fetch_assoc();
        if ($student && password_verify($password, $student['password'])) {
            $_SESSION['student_id']     = $student['id_number'];
            $_SESSION['student_name']   = $student['first_name'].' '.$student['last_name'];
            $_SESSION['student_course'] = $student['course'];
            header('Location: '.STUDENT_URL.'dashboard.php'); exit();
        } else { $error = 'Invalid Student ID or Password.'; }
    } else { $error = 'Please enter your Student ID and Password.'; }
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Student Login | ESTI CGMS</title>
...
<div class="brand-panel">
    <div class="brand-top">
        ...
        <h1 class="brand-heading">Your record,<br><em>at a glance.</em></h1>
        <p class="brand-sub">Sign in to view your grades, class schedule, and academic standing.</p>
    </div>
...
<div class="form-panel">
    <div class="form-wrap">
        <div class="role-switch">
            ...
            <a href="<?= BASE_URL ?>faculty/login.php">Faculty</a>
        </div>

        <div class="form-eyebrow">Student access</div>
        <h2 class="form-title">Welcome back</h2>
        <p class="form-desc">Enter your credentials to view your grades and records.</p>
        ...
        <form method="POST">
            <div class="field">
                ...
                <div class="field-input">
                    ...
                    <input type="text" name="id_number" id="id_number" placeholder="Enter your Student ID (e.g. 20241001)" value="<?= htmlspecialchars($_POST['id_number'] ?? '') ?>" required autofocus>
                </div>
            </div>
            <div class="field">
